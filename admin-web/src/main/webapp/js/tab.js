...
layui.define(['element', 'common'], function(exports) {
	 ...
	 * @param id
	 */
	Tab.prototype.getTabId=function(id){
		var that = ELEM.titleBox === undefined ? this.init() : this,
			tabId = -1;
		ELEM.titleBox.find('li').each(function(i, e) {
			var $cite = $(this).children('cite');
	...
	Tab.prototype.tabAdd = function(data) {
		var that = this;
		var _config = that.config;
		var tabIndex = that.exists("tab_"+data.id);
		if(tabIndex === -1) {
			//设置只能同时打开多少个tab选项卡
			if(_config.maxSetting !== 'undefined') {
				...
				if(typeof _config.maxSetting === 'object') {
					var max = _config.maxSetting.max || 15;
			...
			title += '<cite id="tab_'+data.id+'" onclick="openMenu('+data.id+')" >' + data.title + '</cite>';
			if(_config.closed) {
			...
			if(_config.closed) {
				...
				ELEM.titleBox.find('li').children('i.layui-tab-close[data-id=' + globalTabIdIndex + ']').on('click', function() {
					...
					if(_config.contextMenu) {
						...
						var tabArr = _config.elem.children('ul.layui-tab-title').children('li');
						for(var i = 0;i < tabArr.length; i++) {
							if($(tabArr[i]).attr('class')=="layui-this"){
								var tab = tabArr[i];
								openMenu($(tab).children('cite').attr('id').split("_")[1]);
								break;
							}
						}						
					}
			...
			//切换到当前打开的选项卡
			element.tabChange(ELEM.tabFilter, that.getTabId("tab_"+data.id));
		} else {
			element.tabChange(ELEM.tabFilter, that.getTabId("tab_"+data.id));
			//自动刷新
